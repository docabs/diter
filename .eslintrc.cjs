...
module.exports = defineConfig({
  ...
  ignorePatterns: ['packages/create-dite/template-**'],
  ...
  rules: {
    ...
    'n/no-extraneous-import': [
      'error',
      {
        allowModules: ['dite', 'less', 'sass', 'vitest', 'unbuild'],
      },
    ],
  ...
  overrides: [
    ...
    },
    {
      ...
      rules: {
        'n/no-restricted-require': [
          'error',
          Object.keys(
            require('./packages/dite/package.json').devDependencies,
          ).map((d) => ({
            name: d,
            message:
    ...
    {
      files: [
        'packages/dite/src/types/**',
        'packages/dite/scripts/**',
        '*.spec.ts',
      ],
      rules: {
    ...
    {
      files: ['packages/create-dite/template-*/**', '**/build.config.ts'],
      rules: {
        'no-undef': 'off',
        'n/no-missing-import': 'off',
