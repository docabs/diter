...
export const assetUrlRE = /__DITE_ASSET__([\w$]+)__(?:\$_(.*?)__)?/g
...
//   // imgElement.src = "__DITE_ASSET__5aA0Ddc0__" are using quotes

//   // Urls added in CSS that is imported in JS end up like
//   // var inlined = ".inlined{color:green;background:url(__DITE_ASSET__5aA0Ddc0__)}\n";
...
//     chunk.diteMetadata!.importedAssets.add(cleanUrl(file))
...
//   // Replace __DITE_PUBLIC_ASSET__5aA0Ddc0__ with absolute paths
...
// const diteBuildPublicIdPrefix = '\0dite:asset:public'
...
//           ? `${diteBuildPublicIdPrefix}${id}`
//           : id
//       }
//     },

//     async load(id) {
//       if (id.startsWith(diteBuildPublicIdPrefix)) {
//         id = id.slice(diteBuildPublicIdPrefix.length)
//       }
...
export const publicAssetUrlRE = /__DITE_PUBLIC_ASSET__([a-z\d]{8})__/g
...
export function publicFileToBuiltUrl(
  ...
  return `__DITE_PUBLIC_ASSET__${hash}__`
}
...
async function fileToBuiltUrl(
  ...
  if (
    ...
    url = `__DITE_ASSET__${referenceId}__${postfix ? `$_${postfix}__` : ``}` // TODO_BASE
  }
  ...
  if (stringContent.includes('<text')) {
    ...
    return (
      ...
      stringContent
        ...
        // https://github.com/ditejs/dite/pull/14643#issuecomment-1766288673
        .replaceAll(/\s+/g, '%20')
    )
  }
}
