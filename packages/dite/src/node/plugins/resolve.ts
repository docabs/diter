...
export const browserExternalId = '__dite-browser-external'
// special id for packages that are optional peer deps
export const optionalPeerDepId = '__dite-optional-peer-dep'

const subpathImportsPrefix = '#'

const startsWithWordCharRE = /^\w/

const debug = createDebugger('dite:resolve-details', {
...
export interface InternalResolveOptions extends Required<ResolveOptions> {
  ...
  scan?: boolean
  // Appends ?__dite_skip_optimization to the resolved id if shouldn't be optimized
...
export function resolvePlugin(resolveOptions: InternalResolveOptions): Plugin {
  ...
  return {
    ...
    async resolveId(id, importer, resolveOpts) {
      ...
      if (resolvedImports) {
        id = resolvedImports
        if (resolveOpts.custom?.['dite:import-glob']?.isSubImportsPattern) {
          return id
        }
      }
      if (importer) {
        if (
          ...
          const moduleLang = this.getModuleInfo(importer)?.meta?.dite?.lang
            ...
            if (!asSrc) {
              ...
              this.warn(
                `Module "${id}" has been externalized for browser compatibility, imported by "${importer}". ` +
                  `See http://ditejs.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.`,
              )
            }
            return isProduction
              ? browserExternalId
  ...
  export default new Proxy({}, {
    get(_, key) {
      throw new Error(\`Module "${id}" has been externalized for browser compatibility. Cannot access "${id}.\${key}" in client code.  See http://ditejs.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.\`)
    }
  })`
        }
      }
...
export function tryNodeResolve(
  ...
  //     return {
  //       id: skipOptimization
  //         ? injectQuery(resolved, `__dite_skip_optimization`)
  //         : resolved,
  //     }
  //   }
  //   if (skipOptimization) {
