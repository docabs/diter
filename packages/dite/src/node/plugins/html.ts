...
// import type { DiteDevServer } from '../server'
import {
  cleanUrl,
  generateCodeFrame,
  getHash,
...
const inlineCSSRE = /__DITE_INLINE_CSS__([a-z\d]{8}_\d+)__/g
...
// PS: key like `hash(/dite/playground/assets/index.html)_1`)
...
export function htmlInlineProxyPlugin(config: ResolvedConfig): Plugin {
  ...
  // https://github.com/ditejs/dite/issues/6372
  htmlProxyMap.set(config, new Map())
  return {
    name: 'dite:html-inline-proxy',
...
export function overwriteAttrValue(
  ...
  if (!valueStart) {
    ...
    throw new Error(
      `[dite:html] internal error, failed to overwrite attribute value`,
    )
  }
  ...
  return {
    ...
    async transform(html, id) {
      if (id.endsWith('.html')) {
        ...
        await traverseHtml(html, id, (node) => {
          ...
          if (inlineStyle) {
            inlineModuleIndex++
            // replace `inline style` with __DITE_INLINE_CSS__**_**__
            ...
            overwriteAttrValue(
              ...
              `__DITE_INLINE_CSS__${hash}_${inlineModuleIndex}__`,
            )
          }
    ...
    async generateBundle(options, bundle) {
      ...
      const getCssTagsForChunk = (
        ...
        chunk.diteMetadata!.importedCss.forEach((file) => {
          if (!seen.has(file)) {
            seen.add(file)
            tags.push({
              tag: 'link',
