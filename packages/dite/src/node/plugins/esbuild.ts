...
// const debug = createDebugger('dite:esbuild')
...
export async function transformWithEsbuild(
  ...
  try {
    ...
    //         ) {
    //           m.text +=
    //             '. Dite 5 now uses esbuild 0.18 and you need to enable them by adding "experimentalDecorators": true in your "tsconfig.json" file.'
    //         }
...
export const buildEsbuildPlugin = (config: ResolvedConfig): Plugin => {
  return {
    name: 'dite:esbuild-transpile',
    async renderChunk(code, chunk, opts) {
      // @ts-expect-error injected by @ditejs/plugin-legacy
      if (opts.__dite_skip_esbuild__) {
        return null
      }
...
//     // server may not be available if dite config is updated at the same time
//     if (server) {
//       // force full reload
//       server.ws.send({
//         type: 'full-reload',
