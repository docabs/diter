...
// const debug = createDebugger('dite:deps')
...
export interface DepOptimizationConfig {
   ...
   * with Dite's dep optimization.
   *
   * - `external` is also omitted, use Dite's `optimizeDeps.exclude` option
   * - `plugins` are merged with Dite's dep plugin
   *
   ...
   *
   * By default, Dite can optimize `.mjs`, `.js`, `.ts`, and `.mts` files. This option
...
export type DepOptimizationOptions = DepOptimizationConfig & {
  /**
   * By default, Dite will crawl your `index.html` to detect dependencies that
   * need to be pre-bundled. If `build.rollupOptions.input` is specified, Dite
   * will crawl those entry points instead.
   *
   * If neither of these fit your needs, you can specify custom entries using
   * this option - the value should be a fast-glob pattern or array of patterns
   * (https://github.com/mrmlnc/fast-glob#basic-syntax) that are relative from
   * dite project root. This will overwrite default entries inference.
   */
...
//  * Used by Dite CLI when running `dite optimize`.
//  */
...
// /**
//  * Internally, Dite uses this function to prepare a optimizeDeps run. When Dite starts, we can get
...
//       ) {
//         throw new Error(
//           `[dite] 'build.rollupOptions.external' can only be an array of strings or a string when using esbuild optimization at build time.`,
//         )
//       }
...
//       // like '/@fs/absolute/path/to/node_modules/.dite'
//       `/@fs/${removeLeadingSlash(normalizePath(depsCacheDir))}`
//     : // if the cache directory is inside root, the url prefix would be something
//       // like '/node_modules/.dite'
