...
// const debug = createDebugger('dite:config')
...
// /**
//  * Type helper to make it easier to use dite.config.ts
...
export interface UserConfig {
   ...
   * deps or some other cache files that generated by dite, which can improve
   * the performance. You can use `--force` flag or manually delete the directory
   * to regenerate the cache files. The value can be either an absolute file
   * system path or a path relative to project root.
   * Default to `.dite` when no `package.json` is detected.
   * @default 'node_modules/.dite'
   */
  cacheDir?: string
  /**
  ...
  //  * future minor version. Please always pin Dite's version to a minor when using them.
  //  */
  ...
  worker?: {
    ...
    format?: 'es' | 'iife'
    /**
     * Dite plugins that apply to worker bundle. The plugins retured by this function
...
//   /**
//    * In Dite 4, SSR-externalized modules (modules not bundled and loaded by Node.js at runtime)
...
export async function resolveConfig(
  ...
  //     process.env.DITE_TEST_WITHOUT_PLUGIN_COMMONJS
  //   ) {
  ...
  //   // loaded from `.staging.env` and set by us as DITE_USER_NODE_ENV
  //   const userNodeEnv = process.env.DITE_USER_NODE_ENV
  ...
  //           `If you need to set process.env.NODE_ENV, you can set it in the Dite config instead.`,
  //       )
  //     }
  //   }
  ...
  const cacheDir = normalizePath(
    config.cacheDir
      ...
      : pkgDir
      ? path.join(pkgDir, `node_modules/.dite`)
      : path.join(resolvedRoot, `.dite`),
  )
  ...
  //           `Please update your Dite config accordingly.\n`,
  //       ),
  //     )
  //   }
  ...
  //     // So we need to separate the worker plugin from the plugin that dite needs to run.
  ...
  //           `Note Dite now defaults to use esbuild for minification. If you still ` +
  ...
  //           colors.yellow(`
  // assetFileNames isn't equal for every build.rollupOptions.output. A single pattern across all outputs is supported by Dite.
  // `),
  //         )
  //       }
  //     }
...
/**
 * Resolve base url. Note that some users use Dite to build for non-web targets like
