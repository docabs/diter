...
/**
 * Dite plugins extends the Rollup plugin interface with a few extra
 * dite-specific options. A valid dite plugin is also a valid Rollup plugin.
 * On the contrary, a Rollup plugin may or may NOT be a valid dite universal
...
export interface Plugin<A = any> extends RollupPlugin<A> {
  ...
  /**
   * Modify dite config before it's resolved. The hook can either mutate the
  ...
  >
  /**
   * Use this hook to read and store the final resolved dite config.
   */
